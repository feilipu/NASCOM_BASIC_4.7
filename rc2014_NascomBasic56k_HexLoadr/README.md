...
For the RC2014 with 56kB of RAM, when the RC2014 does a cold start it requests the "Memory Top?" figure. Setting this to 36863 (`0x8FFF`), or lower, will give you space from `0x9000` to `0xFFFF` for your program.
...
For the RC2014 with 56k Basic the `USR(x)` jump address is located at `0x2224`.
For example, if the origin of your arbitrary program is located at `0x9000` then the Basic command to set the `USR(x)` jump address is `DOKE &h2224, &h9000`.
...
2. Start your 56k RAM RC2014 with the `Memory top?` set to 36863 (`0x8FFF`) or lower. This leaves space for your program from `0x9000` through to `0xFFFF`. Adjust this if needed to suit your individual needs.
...
5. When HexLoadr has finished, and you are back at the Basic `ok` prompt, use the `DOKE` command relocate the address for the Basic `USR(x)` command to point to `.ORG` of your arbitrary program. For the RC2014 the `USR(x)` jump address is located at `0x2224`. If your arbitrary program is located at `0x9000` then the Basic command is `DOKE &h2224, &h9000`, for example.
