...
Your assembly program can receive a 16 bit parameter passed in from the function by calling `DEINT` at `0x0B03`. The parameter is stored in register pair `DE`.

When your assembly program is finished it can return a 16 bit parameter stored in `A` (MSB) and `B` (LSB) by jumping to `ABPASS` which is located at `0x12DC`.
...
DEINT           .EQU    $0B03   ; Function DEINT to get USR(x) into DE registers
ABPASS          .EQU    $12DC   ; Function ABPASS to put output into AB register for return
