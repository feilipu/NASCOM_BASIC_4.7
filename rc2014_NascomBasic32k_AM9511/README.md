...
## RC2014 Mini, Micro, Classic with Am9511A APU Module

This ROM works with most basic versions of the RC2014, with 32k of RAM and 8kb of ROM. This is the ROM to choose if you want fast I/O from a standard RC2014, together with the capability to upload and run C or assembly programs from within Basic.
...
Your assembly program can receive a 16 bit parameter passed in from the function by calling `DEINT` at `0x0AEF`. The parameter is stored in register pair `DE`.

When your assembly program is finished it can return a 16 bit parameter stored in `A` (MSB) and `B` (LSB) by jumping to `ABPASS` which is located at `0x1282`.

Note that these address of these functions can also be read from `0x024B` for `DEINT` and `0x024A` for `ABPASS`, as noted in the NASCOM Basic Manual.

``` asm
                                ; from Nascom Basic Symbol Tables
DEINT           .EQU    $0AEF   ; Function DEINT to get USR(x) into DE registers
ABPASS          .EQU    $1282   ; Function ABPASS to put output into AB register for return
