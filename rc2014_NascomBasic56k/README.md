...
When your assembly program is finished it can return a 16 bit parameter stored in `A` (MSB) and `B` (LSB) by jumping to `ABPASS` which is located at `0x128C`.
...
ABPASS          .EQU    $128C   ; Function ABPASS to put output into AB register for return
